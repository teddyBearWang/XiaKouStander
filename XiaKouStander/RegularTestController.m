...
#import "RegularTestController.h"
...
@interface RegularTestController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
    NSArray *_list;//数据源
...
@implementation RegularTestController
...
- (void)viewDidLoad {
    ...
    _list = @[@"序号",@"工器具名称",@"使用单位",@"送检人",@"试验单位/部门",@"试验结果",@"试验人",@"备注",];
    //默认为当天的日期
...
{
    NSLog(@"新增处理结果");
    [self performSegueWithIdentifier:@"addTestNote" sender:nil];
}
